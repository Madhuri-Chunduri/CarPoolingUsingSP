  ...
  appendViaPoint() {
    ...
    if (this.state.viaPointCount < 4) {
      this.setState((prevState) => ({
        inputs: prevState.viaPoints.concat([newInput]),
        viaPointCount: newInput,
      }));
      // this.setState({addViaPoint : true,  showAvailableSeats : false, viaPointCount : viaPointCount});
    }
  }
  ...
  addViaPoints(rideId) {
    for (let index = 0; index < this.state.viaPointCount; index++) {
      sp.web.lists.getByTitle("ViaPoint").items.add({
        RideIdId: rideId,
        Index: 1,
        StopName: this.state.stops[index],
      });
    ...
    return (
      <div className="viaPointField">
        ...
        <input
          ...
          onChange={(event) => this.handleStop(this.props.index, event)}
          name={name}
        />
      </div>
    );
...
export default AddRideComponent;
